...
$(document).ready(function () {
    ...
    // サーバに接続
    server_connect("ws://localhost:39605/wsserver")
    ...
    // ウィンドウリサイズ時に出力エリアの高さを動的に更新
    function updateOutareaHeight() {
        ...
        const totalUsedHeight = headerHeight + menuHeight + inputAreaHeight + 40; // 40pxは余白
        
        const outarea = document.querySelector('.outarea');
        if (outarea) {
            const newHeight = window.innerHeight - totalUsedHeight;
            outarea.style.height = Math.max(200, newHeight) + 'px';
        }
    }
    
    // 初期化時と、ウィンドウリサイズ時に高さを更新
    updateOutareaHeight();
    window.addEventListener('resize', updateOutareaHeight);


    
    // ズーム時にもoutareaのサイズを再度固定する
    $(document).on('wheel', function (e) {
        if (e.ctrlKey || e.metaKey) {
            // Ctrl+スクロールでズームされている
            setTimeout(updateOutareaHeight, 50);
        }
    });
    // ウインドウの位置
    $(function () {
